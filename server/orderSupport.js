...
// 1) пробуем из req.user (companyId/company_id)
// 2) если нет — пытаемся по userId/id найти в таблице users
async function resolveCompanyContext(req, res) {
    ...
    if (!companyId) {
        ...
        // пробуем id
        let rows;
        [rows] = await pool.query("SELECT company_id FROM users WHERE id=? LIMIT 1", [userId]);
        if (!rows.length) {
            // пробуем user_id (если у тебя другая колонка)
        ...
        req.user = { ...u, companyId }; // сохраним для следующих обработчиков
    }
...
export async function getCouriers(req, res) {
    try {
        ...
        res.json({
            ok: true,
            items: rows.map(r => ({ id: r.unit_id, nickname: r.unit_nickname }))
        });
    } catch (e) {
...
export async function searchMenuItems(req, res) {
    try {
        ...
        let sql =
            ...
            "FROM menu WHERE company_id=? AND is_active=1";
